...
const sampleValue = ref("Loading...");

let testAPIFunction = async () => {
	try {
		let params = {};
		params.sampleValue = "123456";
		const response = await axios.post('/api/testEndpoint', params);
		sampleValue.value = response.data['sampleValue'];
	}
	catch(error) {
		await SharedUtils.log(`Error: ${error}`);
		return false;
	}
}
